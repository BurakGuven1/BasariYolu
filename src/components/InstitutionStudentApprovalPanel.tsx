    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Öğrenci onayları</h2>
        ...
        <button
          ...
          className="rounded-lg border border-gray-200 px-3 py-1 text-xs text-gray-600 hover:border-gray-300 hover:text-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:text-gray-100"
        >
          Yenile
        </button>
      </div>
      ...
      {error && (
        <p className="mt-3 rounded border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700 dark:border-red-900 dark:bg-red-900/30 dark:text-red-200">
          {error}
        </p>
      )}

      {loading ? (
        <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">Öğrenciler yükleniyor...</p>
      ) : (
      ...
      {requests.length === 0 ? (
        ...
        <ul className="mt-3 space-y-3">
          {requests.map((request) => (
            <li
              key={request.id}
              className="rounded-xl border border-gray-100 p-4 dark:border-gray-800 dark:bg-gray-900/40"
            >
                ...
                {variant ? (
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-lg border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-500 hover:border-red-300 hover:text-red-600 disabled:opacity-50 dark:border-red-900 dark:text-red-300"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-lg border border-green-200 px-3 py-1.5 text-xs font-semibold text-green-600 hover:border-green-300 hover:text-green-700 disabled:opacity-50 dark:border-green-900 dark:text-green-300"
                    >
...
function StatusChip({ variant }: { variant: 'approved' | 'rejected' }) {
  if (variant === 'approved') {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-green-50 px-3 py-1 text-xs font-semibold text-green-600 dark:bg-green-900/30 dark:text-green-200">
        <Check className="h-3.5 w-3.5" />
        Onaylandı
      </span>
    );
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-600 dark:bg-red-900/30 dark:text-red-200">
      <X className="h-3.5 w-3.5" />
      Reddedildi
    </span>
  );
