...
export default function StudentExamPerformancePanel({ userId }: StudentExamPerformancePanelProps) {
  ...
  useEffect(() => {
    ...
    if (!isValidUuid(userId)) {
      ...
      setSummary(null);
      setLoading(false);
      setError('Geçerli bir öğrenci bilgisi bulunamadı. Lütfen tekrar giriş yapın.');
      return;
    }
    ...
    const loadData = async () => {
      ...
      try {
        ...
        setError(getErrorMessage(error, 'Performans verileri alınamadı.'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const chartData = useMemo(() => {
    ...
    return [...summary.last_scores]
      ...
      .reverse()
      .map((item, index) => ({
        label: item.examName || `Sınav ${index + 1}`,
    ...
    const breakdownItems = summary.source_breakdown
      ?.map(
        ...
        <li>
          ...
          <strong>${item.count} kayıt</strong>
...
<body>
  <section>
    <h1>Öğrenci Performans Raporu</h1>
    <p>${new Date().toLocaleString('tr-TR')}</p>
    <div class="stats">
      <div class="card"><span>Toplam Sınav</span><strong>${summary.total_exams}</strong></div>
      <div class="card"><span>Ortalama Skor</span><strong>${safeScore(summary.average_score)}</strong></div>
      <div class="card"><span>En İyi Skor</span><strong>${safeScore(summary.best_score)}</strong></div>
    </div>
  </section>
  <section>
    <h2>Skor Trendi</h2>
    ${chartBars || '<p>Grafik verisi bulunamadı.</p>'}
  </section>
  <section>
    <h2>Kaynak Dağılımı</h2>
    <ul>${breakdownItems || '<li>Veri bulunamadı.</li>'}</ul>
  </section>
  <section>
    <h2>Öne Çıkan Metrikler</h2>
    <table>
      <thead>
        <tr><th>Sınav</th><th>Kaynak</th><th>Skor</th><th>D/Y/B</th></tr>
      </thead>
  ...
  <section>
    ...
    <ul>${artifactRows || '<li>Henüz belge yüklenmemiş.</li>'}</ul>
  </section>
</body>
</html>`;
  ...
  return (
    <section className="space-y-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-wide text-gray-500">Kurum Deneme Performansı</p>
          <h3 className="text-lg font-semibold text-gray-900">Sonuçlarım ve Belgelerim</h3>
        </div>
        <button
          type="button"
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Toplam Sınav" value={summary?.total_exams ? String(summary.total_exams) : '0'} icon={Activity} />
        <StatCard label="Ortalama Skor" value={formatScore(summary?.average_score) ? `%${formatScore(summary?.average_score)}` : '•'} icon={TrendingUp} />
        <StatCard label="En İyi Skor" value={formatScore(summary?.best_score) ? `%${formatScore(summary?.best_score)}` : '•'} icon={FileText} />
      </div>

      <div className="grid gap-6 lg:grid-cols-[2fr,1fr]">
        <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
          <p className="text-xs uppercase tracking-wide text-gray-500">Skor grafiği</p>
          {chartData.length === 0 ? (
            <p className="mt-3 text-sm text-gray-500">Henüz skor kaydı bulunmuyor.</p>
          ) : (
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-4">
          <p className="text-xs uppercase tracking-wide text-gray-500">Kaynak dağılımı</p>
            ...
            <ul className="mt-3 space-y-2 text-sm text-gray-700">
              {summary.source_breakdown.map((item) => (
                <li key={`${item.source}-${item.count}`} className="flex items-center justify-between text-xs">
                  ...
                  <span>
                    {item.count} sınav • {item.average ? `%${Math.round(item.average)}` : 'skor yok'}
                  </span>
                </li>
              ))}
            </ul>
          ) : (
            <p className="mt-3 text-sm text-gray-500">Kaynak detayları henüz yok.</p>
          )}
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-gray-100 bg-white p-4">
          ...
          {artifacts.length === 0 ? (
            <p className="mt-2 text-sm text-gray-500">Henüz kurum tarafından paylaşılmış belge yok.</p>
          ) : (
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-4">
          ...
          {metrics.length === 0 ? (
            <p className="mt-2 text-sm text-gray-500">Henüz metrik kaydı bulunmuyor.</p>
          ) : (
            <table className="mt-3 w-full text-xs text-gray-600">
              <thead>
                <tr>
                  <th className="pb-2 text-left font-semibold">Sınav</th>
