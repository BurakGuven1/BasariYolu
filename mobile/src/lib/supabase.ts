...
export const connectParentToStudent = async (parentId: string, inviteCode: string) => {
  ...
  const { data, error } = await supabase.from('parent_student_connections').insert([
    {
      parent_id: parentId,
...
export const addExamResult = async (examData: any) => {
  ...
  const { data, error } = await supabase.from('exam_results').insert([examData]).select();
...
export const addHomework = async (homeworkData: any) => {
  const { data, error } = await supabase.from('homeworks').insert([homeworkData]);
  return { data, error };
};

export const updateHomework = async (homeworkId: string, updates: any) => {
  const { data, error } = await supabase.from('homeworks').update(updates).eq('id', homeworkId);
  return { data, error };
};

export const deleteExamResult = async (examId: string) => {
  const { data, error } = await supabase.from('exam_results').delete().eq('id', examId);
  return { data, error };
};

export const updateExamResult = async (examId: string, updates: any) => {
  const { data, error } = await supabase.from('exam_results').update(updates).eq('id', examId);
  return { data, error };
};

export const deleteHomework = async (homeworkId: string) => {
  const { data, error } = await supabase.from('homeworks').delete().eq('id', homeworkId);
  return { data, error };
};
...
export const createWeeklyStudyGoal = async (goalData: any) => {
  const { data, error } = await supabase.from('weekly_study_goals').insert([goalData]);
  return { data, error };
};
...
export const getWeeklyStudySessions = async (
  studentId: string,
  startDate: string,
  endDate: string
) => {
...
export async function savePomodoroSession(sessionData: {
  ...
  const { data, error } = await supabase.from('pomodoro_sessions').insert([sessionData]);
