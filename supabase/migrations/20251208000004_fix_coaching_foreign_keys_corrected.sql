-- =====================================================
-- FIX COACHING SYSTEM FOREIGN KEYS (CORRECTED)
-- =====================================================
-- This migration fixes foreign key references to use profiles.id
...
-- Add new foreign keys referencing profiles table via id
-- profiles.id is the primary key that also serves as the auth user id
...
-- (They already use auth.uid() which matches profiles.id, so no changes needed)

COMMENT ON CONSTRAINT coaching_appointments_coach_id_fkey ON public.coaching_appointments
IS 'Foreign key to profiles table via id';
