-- FIX v2: Institution student signup RLS - Allow signup without session
-- Problem: Email confirmation açık olduğunda yeni user'ın session'ı yok, auth.uid() null
-- Solution: INSERT için authenticated + anon kullanıcılara izin ver
...
-- INSERT policy: Allow authenticated AND anonymous users to insert
-- Yeni kayıt sırasında session olmayabileceği için anon'a da izin veriyoruz
CREATE POLICY "institution_student_requests_insert_policy"
  ON institution_student_requests
  FOR INSERT
  TO authenticated, anon
  WITH CHECK (true);  -- Her türlü INSERT'e izin ver
